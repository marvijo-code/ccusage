      ...
      treefmt = {
        ...
        settings.formatter = {
          ...
          oxfmt = {
            ...
            includes = [ "*" ];
            # Generated by `npm install --package-lock-only` (see
            # `just update-publint`); keep npm's own formatting so the committed
            # file matches what `buildNpmPackage` hashes into `npmDepsHash`.
            excludes = [ "nix/publint-package-lock.json" ];
            priority = 4;
          };
          actionlint = {
